...
import { Mountains, Clock } from '@phosphor-icons/react';
      ...
      <div className="px-8 py-6">
        ...
        {!q.isLoading && list.length === 0 && (
          <div className="card p-8 text-center">
            <div className="font-serif italic text-2xl text-ink-2">&quot;Aab-e-Hayat — the trails are quiet.&quot;</div>
            <div className="text-xs text-ink-3 mt-2 tracking-wider font-mono">NO TRACK RECORDINGS YET</div>
          </div>
        )}
            ...
            <tbody className="divide-y divide-line">
              {list.map((t) => (
                <tr key={t.id} className="hover:bg-pashmina/30 transition">
                  ...
                  <td className="px-4 py-3 text-ink-2 font-mono text-xs">{t.trek_slug ?? '—'}</td>
                  <td className="px-4 py-3 text-right font-mono">{(t.distance_m / 1000).toFixed(1)} km</td>
                  <td className="px-4 py-3 text-right font-mono">
                    <Clock size={11} weight="duotone" className="inline mr-1 -mt-0.5" />
                    {fmtDuration(t.duration_s)}
                  </td>
                  <td className="px-4 py-3 text-right font-mono">{t.gain_m.toLocaleString()} m</td>
                  <td className="px-4 py-3 text-right font-mono">
                    {t.max_altitude_m ? (
                      <>
                  ...
                  <td className="px-4 py-3 text-right">
                    {t.is_public ? (
                      <span className="pill pill-saffron">PUBLIC</span>
                    ) : (
